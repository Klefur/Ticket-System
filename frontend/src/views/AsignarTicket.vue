...
const getTickets = async () => {
  const { data: tickets } = await http.get('/ticket/area/' + user.area.id + 'estado' + 1)

  return tickets
}

const getAnalistas = async () => {
  const { data: analistas } = await http.get('/usuario/area/' + user.area.id + 'rol' + 2)
...
.priority-options {
  ...
  flex-direction: column;
  right: 4.5rem;
  /* Alineación a la derecha con respecto al botón "Asignar Prioridad" */
}
